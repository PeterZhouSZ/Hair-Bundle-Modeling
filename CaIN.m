function [CaIN] = CaIN(onset,offset,duration,I,r,bathCa)
...
for i = 1:1:duration
    t = 0.01:0.01:offset-onset;         % time vector of correct duration
    C =    (((0.12 * I) / (4 * pi * 8e-10 * 1.602e-19 * r)) *   erfc(r./(2*(sqrt(8e-10*t)))) / 6.022e23 / 1000);
    Coff = (((0.12 * I) / (4 * pi * 8e-10 * 1.602e-19 * r)) * ( erfc(r./(2*(sqrt(8e-10*t)))) - erfc(r./(2*sqrt((8e-10)*(t-t(1))))) )) /6.022e23/1000;
    ...
    if (i > onset & i < offset)
        ...
        concentration(i) = C_down(i-onset);
    end
    
    if (i >= offset & i < offset+10)    % I allow it 10 msec to come down, and then dictate the [Ca] until the end with the next if statement
        Coff_down = downsample(Coff,100);
        concentration(i) = Coff_down(i-(offset-1));
    end
