function [T,Y]=Martin2003Integrator(delta_t,t_end)
...
[T,Y] = ode45(@MartinDerivatives,tspan,start,options);

%options=optimset('Display','iter');   % Option to display output
%rootstart = [10 10 10 .1 .1];
%[y,dy] = fsolve(@FP,rootstart)

    function [dy] = MartinDerivatives(t,y)
    ...
    Ica = (Po(y(1),y(3),d,gamma,x_c,boltzmann,temp,y(5)) * 20*Pca * zCa^2 * electron * Faraday * Vm .* CaOUTinterp/1000) ./ (boltzmann*temp*(1 - exp((zCa*electron*Vm)/boltzmann*temp)));
    CaIN = -Ica ./ (2*pi*zCa*Faraday*Dca*rm);
    %Final_Ca = CaIN;
    Final_Ca = interp1(tspan,CaOUT,t);
    ...
    dy(4) = k_on_m  * Final_Ca * (1 - y(4)) - k_off_m  * y(4);
    dy(5) = k_on_re * Final_Ca * (1 - y(5)) - k_off_re * y(5);
